...
public class WelcomeService extends BaseService {
	...
	@Produces({ MediaType.APPLICATION_JSON })
	public Response welcomeId(@PathParam(Constants.Services.WelcomeService.WelcomeId.PARAM_ID) Integer pINId) {
		try {
			WelcomePojo lOBWelcomePojo = getWelcomeBO().welcomeId(pINId);
			return Response.status(Response.Status.OK.getStatusCode()).entity(lOBWelcomePojo).build();
